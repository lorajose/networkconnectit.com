...
test("client-safe reports never expose pricing", () => {
  const profile = resolveDesignReportProfile({
    clientSafe: true,
    includePricing: true,
    sections: ["COVER", "PRICING_SUMMARY"],
  });
...
test("default client report contains customer-ready non-pricing sections", () => {
  assert.deepEqual(DEFAULT_CLIENT_DESIGN_REPORT_PROFILE.sections, [
    "COVER",
    "FLOOR_PLANS",
    "CAMERA_COVERAGE",
    "BOM",
...
test("visible layer selection filters report elements", () => {
  ...
  const elements = [
    { id: "camera-1", layerId: "cctv" },
    { id: "door-1", layerId: "access" },
    { id: "legacy", layerId: null },
  ];

  assert.deepEqual(
    filterReportElementsByLayer(elements, profile).map((element) => element.id),
    ["camera-1", "legacy"],
  );
});
