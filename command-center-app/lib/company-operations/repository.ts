...
export async function getOperationsSnapshot(actor: OperationsActor, requestedOrganizationId?: string) {
  ...
  const [technicians, projects, workOrders, schedule, timeEntries, invoices, invoiceLines, expenses, projectProfitability, totals, operationalAlerts] = await Promise.all([
    ...
    prisma.$queryRaw<Array<{ id: string; projectInstallationId: string; title: string; status: string }>>(Prisma.sql`
      SELECT id, projectInstallationId, title, status
    ...
    prisma.$queryRaw<Array<{ alertType: string; severity: string; entityId: string; title: string; detail: string; sortAt: Date }>>(Prisma.sql`
      SELECT 'OVERDUE_INVOICE' AS alertType, 'HIGH' AS severity, id AS entityId,
        CONCAT('Invoice ', invoiceNumber, ' is overdue') AS title,
        CONCAT(customerName, ' has an outstanding balance') AS detail,
        COALESCE(dueDate, createdAt) AS sortAt
      FROM OperationsInvoice
      WHERE organizationId = ${organizationId} AND status = 'OVERDUE' AND paidAmount < totalAmount
      UNION ALL
      SELECT 'UNASSIGNED_WORK_ORDER', 'MEDIUM', id,
        CONCAT('Unassigned WorkOrder: ', title),
        CONCAT('Status ', status, ' · project ', projectInstallationId),
        updatedAt
      FROM ProjectWorkOrder
      WHERE organizationId = ${organizationId}
      ...
      UNION ALL
      SELECT 'UPCOMING_ASSIGNMENT', 'INFO', s.id,
        CONCAT('Upcoming assignment: ', s.title),
        CONCAT(t.displayName, ' · starts within 48 hours'),
        s.startsAt
      FROM OperationsScheduleEntry s
      JOIN FieldTechnicianProfile t ON t.id = s.technicianProfileId AND t.organizationId = s.organizationId
      WHERE s.organizationId = ${organizationId}
        AND s.status = 'SCHEDULED' AND s.entryType = 'ASSIGNMENT'
        AND s.startsAt >= NOW() AND s.startsAt < DATE_ADD(NOW(), INTERVAL 48 HOUR)
