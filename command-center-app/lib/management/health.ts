...
export async function simulateHealthCheck(user:TenantUser,deviceId:string,nextStatus?:HealthStatus){if(!canRunHealthSimulation(user.role))throw new Error("You do not have permission to run health simulations.");return prisma.$transaction(async tx=>{const device=await tx.device.findFirst({where:{id:deviceId,...getScopedRecordWhere(user)},select:{id:true,name:true,organizationId:true,siteId:true,status:true,healthChecks:{orderBy:{checkedAt:"desc"},take:1,select:{status:true}}}});if(!device)throw new Error("Device not found.");await createSimulatedHealthCheck(tx,{...device,latestHealthStatus:device.healthChecks[0]?.status??healthStatusFromDeviceStatus(device.status)},user.id,nextStatus);});}
export async function simulateSiteHealthCheck(user:TenantUser,siteId:string){if(!canRunHealthSimulation(user.role))throw new Error("You do not have permission to run health simulations.");return prisma.$transaction(async tx=>{const devices=await tx.device.findMany({where:{siteId,...getScopedRecordWhere(user)},select:{id:true,name:true,organizationId:true,siteId:true,status:true,healthChecks:{orderBy:{checkedAt:"desc"},take:1,select:{status:true}}}});for(const device of devices)await createSimulatedHealthCheck(tx,{...device,latestHealthStatus:device.healthChecks[0]?.status??healthStatusFromDeviceStatus(device.status)},user.id);});}
