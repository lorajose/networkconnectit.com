...
export type DesignReportCommercialLink = {
  estimateId?: string;
  proposalId?: string;
};
...
export type IssuedDesignReport = {
  ...
  commercialLink: DesignReportCommercialLink;
  previousIssueId?: string;
  digest: string;
};

type IssueDesignReportInput = Omit<IssuedDesignReport, "id" | "version" | "issuedAt" | "digest" | "previousIssueId"> & {
  previousIssue?: IssuedDesignReport;
  issuedAt?: string;
};
...
export function issueDesignReport(input: IssueDesignReportInput): IssuedDesignReport {
  ...
  const immutablePayload = {
    ...
    model: input.model,
    commercialLink: input.commercialLink,
    previousIssueId: input.previousIssue?.id,
  };
  const digest = createHash("sha256").update(stable(immutablePayload)).digest("hex");
  return {
    ...immutablePayload,
    id: `design-report:${input.projectId}:v${version}:${digest.slice(0, 16)}`,
    digest,
  };
}
...
export function verifyIssuedDesignReport(report: IssuedDesignReport): boolean {
  const payload = {
    ...
    model: report.model,
    commercialLink: report.commercialLink,
    previousIssueId: report.previousIssueId,
  };
