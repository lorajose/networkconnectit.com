...
import { addBidDocumentMetadata, createBidWorkspace, updateBidWorkspace } from "@/lib/contractor-os/bid-repository";
...
export async function updateBidAction(formData: FormData) {
  ...
  await updateBidWorkspace(actor(user), { organizationId, bidId, title: formString(formData, "title"), status: formString(formData, "status") as any, revision: formString(formData, "revision") || null, bidDueAt: formString(formData, "bidDueAt") || null, notes: formString(formData, "notes") || null, projectInstallationId: formString(formData, "projectInstallationId") || null, estimateId: formString(formData, "estimateId") || null });
