...
import {
  addBidDocumentMetadata,
  createBidWorkspace,
  getBidWorkspace,
  updateBidWorkspace,
  type BidStatus,
} from "@/lib/contractor-os/bid-repository";
...
import { deletePrivateBidFile, storePrivateBidFile } from "@/lib/contractor-os/private-bid-storage";
...
export async function createBidAction(formData: FormData) {
  ...
  const bidId = await createBidWorkspace(actor(user), {
    organizationId,
  ...
  revalidatePath("/bids");
  redirect(`/bids/${bidId}?organizationId=${encodeURIComponent(organizationId)}`);
}
...
export async function updateBidAction(formData: FormData) {
  ...
  await updateBidWorkspace(actor(user), {
    organizationId,
    bidId,
    title: formString(formData, "title"),
    status: formString(formData, "status") as BidStatus,
    revision: formString(formData, "revision") || null,
    bidDueAt: formString(formData, "bidDueAt") || null,
    notes: formString(formData, "notes") || null,
    projectInstallationId: formString(formData, "projectInstallationId") || null,
...
export async function addBidDocumentAction(formData: FormData) {
  ...
  await addBidDocumentMetadata(actor(user), organizationId, bidId, {
    fileName: formString(formData, "fileName"),
    documentType: formString(formData, "documentType") as BidDocumentType,
    revision: formString(formData, "revision") || null,
    sourceKey: formString(formData, "sourceKey"),
    sourceMimeType: formString(formData, "sourceMimeType") || null,
    sourceSizeBytes: formString(formData, "sourceSizeBytes") ? Number(formString(formData, "sourceSizeBytes")) : null,
  });

  revalidatePath(`/bids/${bidId}`);
}

export async function uploadBidDocumentAction(formData: FormData) {
  const user = await requireUser();
  const organizationId = formString(formData, "organizationId");
  const bidId = formString(formData, "bidId");
  const documentType = formString(formData, "documentType") as BidDocumentType;
  const revision = formString(formData, "revision") || null;
  const uploaded = formData.get("file");

  if (!(uploaded instanceof File) || !uploaded.name) {
    throw new Error("Select a bid document to upload");
  }

  const bid = await getBidWorkspace(actor(user), bidId, organizationId);
  if (!bid) throw new Error("Bid workspace not found");

  const stored = await storePrivateBidFile({
    organizationId,
    bidWorkspaceId: bidId,
  ...
  try {
    ...
    await deletePrivateBidFile(stored.sourceKey);
    throw error;
  }
